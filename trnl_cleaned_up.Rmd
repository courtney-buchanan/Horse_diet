...
# new colors for Loasassceae, Malvaceae, Potamogetonaceae

#plot
herd_family_abundance <- plot_bar(ps_family_percent, fill = "Family", title="Herd Average Diet Composition") + xlab("Season") + ylab("Relative Abundance (%)") + theme_bw()+ scale_fill_manual(values= c("#FFB500",  "#A30059", "#1CE6FF", "#0000A6",  "#FEFFE6","#BA0900","#5A0007", "#008941","#FF34FF",  "#3B5DFF","#FF90C9","#000035", "#C2FF99",   "#999999", "#6600CC", "#A079BF", "#FF4A46","#C2FFED","#D16100", "#4FC601", "#FFAA92","#00489C","#663300","#FFFF00", "#CC0744","#0AA6D8", "#D790FF",
  "#B903AA"))

herd_family_abundance + theme(text = element_text(size = 14, family= "Times New Roman")) + theme(legend.text = element_text(size = 14, color="black")) + theme(axis.text = element_text(size = 14, color= "black")) + theme(axis.title = element_text(size = 14, color= "black"))+
theme(plot.title = element_text(hjust=0.5)) +geom_bar(stat="identity", size=2)

ggsave("overall_herd_diet2.png", width=22, height =8, dpi = 300)
...
#New color for Potagmogacenaceae
#plot
season_family_abundance2 <- plot_bar(ps_family_range_percent, fill = "Family", title="Rangewide Average Diet Composition") + xlab("Season") + ylab("Relative Abundance (%)") + theme_bw()+ scale_fill_manual(values= c("#FFB500", "#1CE6FF",  "#FEFFE6", "#008941","#FF34FF",  "#3B5DFF","#FF90C9", "#C2FF99", "#4FC601", "#00489C","#663300","#FFFF00", "#CC0744", "#D790FF", "#B903AA"))

season_family_abundance2 + theme(text = element_text(size = 14, family= "Times New Roman")) + theme(legend.text = element_text(size = 14, color="black")) + theme(axis.text = element_text(size = 14, color= "black")) + theme(axis.title = element_text(size = 14, color= "black"))+
theme(plot.title = element_text(hjust=0.5)) +geom_bar(stat="identity", size=2)

ggsave("overall_season_diet2.png", width=8, height =8, dpi = 300)
...
###New colors for Malvaceae
# Plot relative abundances
ATfamily_abundance3 <- plot_bar(ATfamily_percent2, fill = "Family", title= "Adobe Town") + xlab("Season") + ylab("Relative Abundance (%)") + theme_bw()+ scale_fill_manual(values= c("#FFB500", "#006FA6", "#1CE6FF",
 "#3A2465",  "#FEFFE6","#5A0007", "#008941",  "#3B5DFF","#FF90C9","#000035", "#C2FF99", "#6600CC", "#C2FFED","#D16100", "#4FC601", "#FFAA92","#00489C","#FFFF00", "#D790FF"))

ATfamily_abundance3 + theme(text = element_text(size = 14, family= "Times New Roman")) + theme(legend.text = element_text(size = 12, color="black")) + theme(axis.text = element_text(size = 8, color= "black")) + theme(axis.title = element_text(size = 14, color= "black"))+
theme(plot.title = element_text(hjust=0.5))+geom_bar(stat="identity", size=2)

ggsave("AT_sample_SBC2.png", width=15, height =6, dpi = 300)
...
#Get outputs ready for microbial plant Mantel tests

```{r}

####ASV level 

ps_common_wild # has 466 samples
View(sample_data(ps_common_wild))# need to remove extra WMP sample....will do once in table form... 
ps_common_wild2 <- subset_samples(ps_common_wild, study.area != "SB") #get rid of SB so we have version with and without SB
ps_common_wild2 #yes SB removed 

#pull out the OTU tables so I can make distance matrices

#all HMAs
plants_otu_table <- data.frame(otu_table(ps_common_wild))
View(plants_otu_table) #have OTU table, need to remove the WMP11
plants_otu_table$sample_id<- row.names(plants_otu_table) #make a row for sample ID
plants_otu_table<-subset(plants_otu_table, sample_id != "WMP11") #get rid of WMP11
View(plants_otu_table) #check removed the WMP11 sample, 465 rows and 285 columns including "sample_id"
plants_otu_table<- plants_otu_table[,1:284]
View(plants_otu_table) #check removed "sample_id" column 

#SB removed
plants_otu_table_no_SB <- data.frame(otu_table(ps_common_wild2))
View(plants_otu_table_no_SB) #have OTU table, need to remove the WMP11
plants_otu_table_no_SB$sample_id<- row.names(plants_otu_table_no_SB) #make a row for sample ID
plants_otu_table_no_SB<-subset(plants_otu_table_no_SB, sample_id != "WMP11") #get rid of WMP11
View(plants_otu_table_no_SB) #check removed the WMP11 sample, 450 rows, 285 columns including "sample_id"
plants_otu_table_no_SB<- plants_otu_table_no_SB[,1:284] #get rid of sample id column
View(plants_otu_table_no_SB) #check removed "sample_id" column 


#write files

write.csv(plants_otu_table, "./3_19_25common_plants_otu.csv", row.names = TRUE)

write.csv(plants_otu_table_no_SB, "./3_19_25common_plants_otu_no_SB.csv", row.names = TRUE)


#### Family level grouped

ps_filtered_family_realtive #view relative abundance family ps object- still has domestic
ps_family_wild<-subset_samples(ps_filtered_family_realtive, domestication == "wild") #still has WMP11
ps_family_wild #domestics gone
ps_family_wild_no_SB<-subset_samples(ps_family_wild, study.area != "SB") #get rid of SB samples so have a version without them 
ps_family_wild_no_SB #sB removed


#pull out the OTU tables so I can make distance matrices
#all HMAs
plants_family_otu_table <- data.frame(otu_table(ps_family_wild))
View(plants_family_otu_table) #have OTU table, need to remove the WMP11
plants_family_otu_table$sample_id<- row.names(plants_family_otu_table) #make a row for sample ID
plants_family_otu_table<-subset(plants_family_otu_table, sample_id != "WMP11") #get rid of WMP11
View(plants_family_otu_table) #check removed the WMP11 sample, 465 rows, 51 columns including "sample_id"
plants_family_otu_table<- plants_family_otu_table[,1:50] #remove sample id column at end
View(plants_family_otu_table) #check removed "sample_id" column 

#SB removed

plants_family_otu_table_no_SB <- data.frame(otu_table(ps_family_wild_no_SB))
View(plants_family_otu_table_no_SB) #have OTU table, 451 samples, need to remove the WMP11
plants_family_otu_table_no_SB$sample_id<- row.names(plants_family_otu_table_no_SB) #make a row for sample ID
plants_family_otu_table_no_SB<-subset(plants_family_otu_table_no_SB, sample_id != "WMP11") #get rid of WMP11
View(plants_family_otu_table_no_SB) #check removed the WMP11 sample, 450 rows, 51 columns including "sample_id"
plants_family_otu_table_no_SB<- plants_family_otu_table_no_SB[,1:50] #remove sample id column at end
View(plants_family_otu_table_no_SB) #check removed "sample_id" column 

#write files

write.csv(plants_family_otu_table, "./3_19_25family_plants_otu.csv", row.names = TRUE)

write.csv(plants_family_otu_table_no_SB, "./3_19_25family_plants_otu_no_SB.csv", row.names = TRUE)


```
# Analysis for icrobial comparison
## Ordination plots 
```{r}

ps_common_wild #check we are working with correct one
View(sample_data(ps_common_wild))
ps_ordinate<- subset_samples(ps_common_wild, collection_time != "44172.59294") #remove WMP11 so it matches the microbe data
ps_ordinate #one less sample
View(sample_data(ps_ordinate)) #check it was WMP 11 that is gone

#create diversity matrices- after rare taxa filtered out, using bray curtis distance and PCoA
bray_PCoA_filt  <- ordinate(ps_ordinate, "PCoA", "bray")



# Season 
season_bray_PCoA <- plot_ordination(ps_ordinate, 
                                         bray_PCoA_filt,
                                         type = "samples",
                                         color = "season", shape = "season") + theme_bw() 

Fig1A <-season_bray_PCoA +geom_point(size=2)+ stat_ellipse() + 
  labs(title = "Diet PCoA by Season") + theme(text = element_text(size = 18, family="Times New Roman")) + theme(plot.title = element_text(hjust = 0.5, size = 18)) + theme(legend.text = element_text(size = 18, color="black")) + theme(axis.text = element_text(size = 18, color= "black")) + theme(axis.title = element_text(size = 18, color= "black")) +scale_color_manual(values=c("#db6d00","#006ddb"), labels= c("Summer", "Winter"), name="Season") + scale_shape_manual(values=c(16,17),name="Season")




bray_PCoA_PB  <- ordinate(ps_PB, "PCoA", "bray")

PB_season_bray_PCoA <- plot_ordination(ps_PB, 
                                         bray_PCoA_PB,
                                         type = "samples",
                                         color ="season", shape = "season") + theme_bw() 
  
PB_season_bray_PCoA + 
  labs(title = "Bray-Curtis Dissimilarity PCoA season PB") + theme(text = element_text(size = 10)) + geom_point(size=2.5) + stat_ellipse()


Fig1C<-PB_season_bray_PCoA +geom_point(size=2)+ stat_ellipse() + 
  labs(title = "Diet PCoA by Season (Palomino Buttes HMA)") + theme(text = element_text(size = 18, family="Times New Roman")) + theme(plot.title = element_text(hjust = 0.5, size = 18)) + theme(legend.text = element_text(size = 18, color="black")) + theme(axis.text = element_text(size = 18, color= "black")) + theme(axis.title = element_text(size = 18, color= "black")) +scale_color_manual(values=c("#db6d00","#006ddb"), labels= c("Summer", "Winter"), name="Season") + scale_shape_manual(values=c(16,17),name="Season")

## HMA averages phyloseq

ps_HMA<- merge_samples (ps_ordinate, "area_season", fun=mean) #merge by study area, each HMA will be one point

ps_HMA #there are now 31 "samples" one for each HMA and season, still 284 taxa 

view(sample_data(ps_HMA)) # check- need new metadata- it got into NAs
View(otu_table(ps_HMA)) #check- looks good
View(tax_table(ps_HMA)) #check-looks good



#constructing new object 
HMA_otu<-otu_table(ps_HMA) #pull otu table from HMA merged phyloseq object
View(HMA_otu) #looks good, row names are codes for area_season 
HMA_taxa<-tax_table(ps_HMA) #pull taxa table from HMA merged phyloseq object
View(HMA_taxa)

meta_HMA_season2<- read.csv("meta_HMA_season.csv", header=TRUE, row.names = 1) #read in HMA average metadata created in microbes R 

View(meta_HMA_season2)#check 

#create ps object with HMA merged file otu and taxa and metadata
ps_HMA2 <- phyloseq(otu_table(HMA_otu),
               sample_data(meta_HMA_season2), tax_table(HMA_taxa))
ps_HMA2 #Check- looks like correct number of taxa and samples from before 284ASV, 31 samples
View(sample_data(ps_HMA2)) #sample metadata in correct form now 

#create diversity matrices- after rare taxa filtered out, using bray curtis distance and PCoA
bray_PCoA_HMA2  <- ordinate(ps_HMA2, "PCoA", "bray")



# Season 
HMA_season_bray_PCoA <- plot_ordination(ps_HMA2, 
                                         bray_PCoA_HMA2,
                                         type = "samples",
                                         color = "season", shape = "season") + theme_bw() 
...
#colored by season
 ...
 Fig1B<-HMA_season_bray_PCoA +geom_point(size=2)+ stat_ellipse() + 
  labs(title = "Diet PCoA by Season (HMA scale)") + theme(text = element_text(size = 18, family="Times New Roman")) + theme(plot.title = element_text(hjust = 0.5, size = 18)) + theme(legend.text = element_text(size = 18, color="black")) + theme(axis.text = element_text(size = 18, color= "black")) + theme(axis.title = element_text(size = 18, color= "black")) +scale_color_manual(values=c("#db6d00","#006ddb"), labels= c("Summer", "Winter"), name="Season") + scale_shape_manual(values=c(16,17),name="Season")


FigureA3<-ggarrange(Fig1A, Fig1B,Fig1C, labels= c("A","B", "C"), ncol=1, nrow=3, common.legend=TRUE, legend="right", font.label = list(size = 24)) #combine Fig A3 A-C

ggsave("multipane_FigA3.png", width=12, height=20, dpi=300) #save

```

##PERMANOVAS to match microbes
```{r}

#full range scale

# pull out the sample metadata
meta <- data.frame(sample_data(ps_ordinate)) 
View(meta) #look at my data 

adonis_table<-data.frame(otu_table(ps_ordinate)) #pull otu table out of ps object
View(adonis_table)#check

# Season 
set.seed(223)
adonis2((adonis_table)~meta$season, na.action=na.omit, method="bray")


#full range scale - HMA averages

adonis_table_HMA<-data.frame(otu_table(ps_HMA2)) #pull otu table out of HMA level ps object
View(adonis_table_HMA)#check

View(meta_HMA_season2) #check metadata 

#season HMA
set.seed(223)
adonis2((adonis_table_HMA)~meta_HMA_season2$season, na.action=na.omit, method="bray")
...
##PERMANOVA just AT
meta_AT<- subset(meta, study.area == 'AT')#make metadata only single study area
View(meta_AT)#check 

adonis_table_AT<-adonis_table[(rownames(meta_AT)),]#subset adonis table to match metadata with just single study area 
View(adonis_table_AT) #should be 30 samples

#season
set.seed(223)
adonis2((adonis_table_AT)~meta_AT$season, na.action=na.omit, method="bray")


##PERMANOVA just CN
meta_CN<- subset(meta, study.area == 'CN')#make metadata only single study area
View(meta_CN)#check 

adonis_table_CN<-adonis_table[(rownames(meta_CN)),]#subset adonis table to match metadata with just single study area 
View(adonis_table_CN) #should be 30 samples

#season
set.seed(223)
adonis2((adonis_table_CN)~meta_CN$season, na.action=na.omit, method="bray")

##PERMANOVA just FR
meta_FR<- subset(meta, study.area == 'FR')#make metadata only single study area
View(meta_FR)#check 

adonis_table_FR<-adonis_table[(rownames(meta_FR)),]#subset adonis table to match metadata with just single study area 
View(adonis_table_FR) #should be 30 samples

#season
set.seed(223)
adonis2((adonis_table_FR)~meta_FR$season, na.action=na.omit, method="bray")



##PERMANOVA just GM
meta_GM<- subset(meta, study.area == 'GM')#make metadata only single study area
View(meta_GM)#check 

adonis_table_GM<-adonis_table[(rownames(meta_GM)),]#subset adonis table to match metadata with just single study area 
View(adonis_table_GM) #should be 30 samples

#season
set.seed(223)
adonis2((adonis_table_GM)~meta_GM$season, na.action=na.omit, method="bray")


##PERMANOVA just LBC
meta_LBC<- subset(meta, study.area == 'LBC')#make metadata only single study area
View(meta_LBC)#check 

adonis_table_LBC<-adonis_table[(rownames(meta_LBC)),]#subset adonis table to match metadata with just single study area 
View(adonis_table_LBC) #should be 30 samples

#season
set.seed(223)
adonis2((adonis_table_LBC)~meta_LBC$season, na.action=na.omit, method="bray")


##PERMANOVA just MP
meta_MP<- subset(meta, study.area == 'MP')#make metadata only single study area
View(meta_MP)#check 

adonis_table_MP<-adonis_table[(rownames(meta_MP)),]#subset adonis table to match metadata with just single study area 
View(adonis_table_MP) #should be 30 samples

#season
set.seed(223)
adonis2((adonis_table_MP)~meta_MP$season, na.action=na.omit, method="bray")



##PERMANOVA just NH
meta_NH<- subset(meta, study.area == 'NH')#make metadata only single study area
View(meta_NH)#check 

adonis_table_NH<-adonis_table[(rownames(meta_NH)),]#subset adonis table to match metadata with just single study area 
View(adonis_table_NH) #should be 30 samples

#season
set.seed(223)
adonis2((adonis_table_NH)~meta_NH$season, na.action=na.omit, method="bray")


##PERMANOVA just ON
meta_ON<- subset(meta, study.area == 'ON')#make metadata only single study area
View(meta_ON)#check 

adonis_table_ON<-adonis_table[(rownames(meta_ON)),]#subset adonis table to match metadata with just single study area 
View(adonis_table_ON) #should be 30 samples

#season
set.seed(223)
adonis2((adonis_table_ON)~meta_ON$season, na.action=na.omit, method="bray")


##PERMANOVA just PB
meta_PB<- subset(meta, study.area == 'PB')#make metadata only single study area
View(meta_PB)#check 

adonis_table_PB<-adonis_table[(rownames(meta_PB)),]#subset adonis table to match metadata with just single study area 
View(adonis_table_PB) #should be 30 samples

#season
set.seed(223)
adonis2((adonis_table_PB)~meta_PB$season, na.action=na.omit, method="bray")


##PERMANOVA just PN
meta_PN<- subset(meta, study.area == 'PN')#make metadata only single study area
View(meta_PN)#check 

adonis_table_PN<-adonis_table[(rownames(meta_PN)),]#subset adonis table to match metadata with just single study area 
View(adonis_table_PN) #should be 30 samples

#season
set.seed(223)
adonis2((adonis_table_PN)~meta_PN$season, na.action=na.omit, method="bray")


##PERMANOVA just RR
meta_RR<- subset(meta, study.area == 'RR')#make metadata only single study area
View(meta_RR)#check 

adonis_table_RR<-adonis_table[(rownames(meta_RR)),]#subset adonis table to match metadata with just single study area 
View(adonis_table_RR) #should be 30 samples

#season
set.seed(223)
adonis2((adonis_table_RR)~meta_RR$season, na.action=na.omit, method="bray")


##PERMANOVA just SC
meta_SC<- subset(meta, study.area == 'SC')#make metadata only single study area
View(meta_SC)#check 

adonis_table_SC<-adonis_table[(rownames(meta_SC)),]#subset adonis table to match metadata with just single study area 
View(adonis_table_SC) #should be 30 samples

#season
set.seed(223)
adonis2((adonis_table_SC)~meta_SC$season, na.action=na.omit, method="bray")


##PERMANOVA just SS
meta_SS<- subset(meta, study.area == 'SS')#make metadata only single study area
View(meta_SS)#check 

adonis_table_SS<-adonis_table[(rownames(meta_SS)),]#subset adonis table to match metadata with just single study area 
View(adonis_table_SS) #should be 30 samples

#season
set.seed(223)
adonis2((adonis_table_SS)~meta_SS$season, na.action=na.omit, method="bray")


##PERMANOVA just SW
meta_SW<- subset(meta, study.area == 'SW')#make metadata only single study area
View(meta_SW)#check 

adonis_table_SW<-adonis_table[(rownames(meta_SW)),]#subset adonis table to match metadata with just single study area 
View(adonis_table_SW) #should be 30 samples

#season
set.seed(223)
adonis2((adonis_table_SW)~meta_SW$season, na.action=na.omit, method="bray")


##PERMANOVA just TP
meta_TP<- subset(meta, study.area == 'TP')#make metadata only single study area
View(meta_TP)#check 

adonis_table_TP<-adonis_table[(rownames(meta_TP)),]#subset adonis table to match metadata with just single study area 
View(adonis_table_TP) #should be 30 samples

#season
set.seed(223)
adonis2((adonis_table_TP)~meta_TP$season, na.action=na.omit, method="bray")
